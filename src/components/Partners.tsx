    ...
    <section id="partners" className="relative py-24 bg-[#020618] overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="text-4xl lg:text-5xl font-black uppercase mb-3">
            ...
            <span className="text-[#6c5ce7]">PARTNER</span>
          </h2>
            ...
            return (
              <div
                ...
                className={`rounded-2xl p-6 border h-64
                  ${isCenter ? 'bg-white/10 border-[#6c5ce7]/50' : 'bg-white/5 border-white/10'}`}
              >
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <span className="text-[#6c5ce7] text-xs uppercase tracking-widest font-bold block mb-1">{p.categoria}</span>
                ...
                <ul className="flex flex-col gap-1.5">
                  ...
                  {p.features.length > 3 && (
                    <li className="text-[#6c5ce7] text-xs font-bold mt-1">+{p.features.length - 3} más →</li>
                  )}
                </ul>
              </div>
            )
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex justify-center gap-4 mt-6">
          <button onClick={prev} className="text-white bg-white/10 hover:bg-[#5848c4]/20 border border-white/20 hover:border-[#6c5ce7]/50 rounded-full w-10 h-10 flex items-center justify-center transition-all text-sm cursor-pointer">←</button>
          <button onClick={next} className="text-white bg-white/10 hover:bg-[#5848c4]/20 border border-white/20 hover:border-[#6c5ce7]/50 rounded-full w-10 h-10 flex items-center justify-center transition-all text-sm cursor-pointer">→</button>
        </div>
        <div className="flex justify-center gap-2 mt-4">
          {partners.map((_, i) => (
            <button key={i} onClick={() => setCurrent(i)} className={`h-2 rounded-full transition-all duration-300 cursor-pointer ${i === current ? 'bg-[#6c5ce7] w-6' : 'bg-white/30 w-2'}`} />
          ))}
        </div>
        ...
        <div className="text-center mt-8">
          <button
            ...
            className="inline-block bg-[#6c5ce7] hover:bg-[#5848c4] active:scale-95 text-white font-black px-8 py-3 rounded-full uppercase tracking-wide cursor-pointer transition-all"
          >
            Quiero ser Partner →
          </button>
        </div>
      ...
      {popup !== null && (
        <Modal onClose={() => setPopup(null)} titleId="partner-modal-title">
          ...
          <span className="text-[#a89cf0] text-xs uppercase tracking-widest font-bold mb-2 block">{partners[popup].categoria}</span>
          ...
          <ul className="flex flex-col gap-2 mb-4">
            {partners[popup].features.map((f, i) => (
              <li key={i} className="flex items-start gap-2 text-gray-200 text-sm">
                <span className="text-[#a89cf0] mt-0.5" aria-hidden>✓</span>{f}
              </li>
            ))}
          </ul>
          ...
          <div className="flex gap-3">
            <button
              ...
              className="bg-[#6c5ce7] hover:bg-[#5848c4] active:scale-95 text-white font-black px-6 py-2 rounded-full transition-all text-sm cursor-pointer"
            >
              Contactar →
            </button>
            <button
