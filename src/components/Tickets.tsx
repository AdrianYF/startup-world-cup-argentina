import { content, type TicketPlan } from '../lib/content'
import { openTicketing } from '../lib/ticketing'

/** Copy del botón según el estado de la tanda. Solo la que está a la venta linkea a Startup Grind. */
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="text-center mb-16">
          ...
          <p className="text-gray-400 text-lg max-w-2xl mx-auto">
            30 cupos por tanda. Cuando se agota una, sube el precio.
          </p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
          {content.tickets.map((plan: TicketPlan) => {
              ...
              <div
                ...
                className={`relative rounded-2xl p-6 sm:p-8 border-[0.5px] transition-all ${
                  plan.badge
                    ...
                    : 'bg-white/5 border-[#75AADB]/10 shadow-[0_0_14px_-8px_rgba(117,170,219,0.1)]'
                } ${agotado ? 'opacity-60' : ''}`}
              >
                {plan.badge && (
                ...
                <button
                  ...
                  style={disponible ? { backgroundImage: 'var(--gradient-cta)' } : undefined}
                  className={`block w-full text-center font-black py-3 rounded-full uppercase tracking-wide transition-all ${
                    disponible
                      ? 'cursor-pointer active:scale-95 text-white [text-shadow:0_1px_2px_rgba(0,0,0,0.3)] hover:scale-105'
